...
# Load new dataset (different bus)
NEW_FILE = r"C:\Users\1000684\Downloads\BCS_fault_prediction\9M_MBMT_NAC_MH04LQ9368_bcs.xlsx"
...
# Predict
print("\nEvaluating model on new bus data...")
y_pred = model.predict(X)

# Report accuracy
accuracy = accuracy_score(y, y_pred)
print(f"\n{'='*60}")
print(f"ACCURACY ON NEW BUS (MH04LQ9368): {accuracy:.4f} ({accuracy*100:.2f}%)")
print(f"{'='*60}")
...
print(f"  Detected: {detected_faults}")
print(f"  Missed: {total_faults - detected_faults}")
print(f"  Detection Rate: {detected_faults/total_faults*100:.2f}%" if total_faults > 0 else "  Detection Rate: N/A (no faults)")
